...
import { GuestRow, GuestGroupLabel, STATUS_ORDER, STATUS_LABEL, type Guest } from "./guest-row";
...
export function GuestList({
  ...
  const grouped = useMemo(() => {
    const groups: Record<string, Guest[]> = { going: [], pending: [], not_attending: [] };
    list
      .filter((g) => !g.isHost)
      .forEach((g) => {
  ...
  const availableCandidates = useMemo(
    () =>
      candidates.filter(
        (c) => !draft.some((g) => g.id === c.id) && c.name.toLowerCase().includes(filter.toLowerCase())
      ),
  ...
  function addCandidate(candidate: NetworkCandidate) {
    setDraft((d) => [...d, { id: candidate.id, name: candidate.name, status: "pending", group: candidate.group }]);
  }
  ...
  return (
    <div>
      ...
      {!hasAnyGuests && !editing && (
        <div className="mt-1.5 flex flex-col items-center gap-1 rounded-2xl border-[1.5px] border-dashed border-joyna-border-strong py-5 text-center text-joyna-ink-faint">
          <HugeiconsIcon icon={UserMultipleIcon} className="h-7 w-7" strokeWidth={1.6} />
        ...
        <div className="mt-1">
          ...
          {availableCandidates.map((c) => (
            <div key={c.id} className="flex items-center gap-2.5 py-2 text-sm">
              ...
              <span className="text-[11px] text-joyna-ink-faint">{c.group}</span>
              <button
                type="button"
              ...
              >
                <HugeiconsIcon icon={PlusSignIcon} className="h-3 w-3 text-joyna-mint-dark" strokeWidth={2.5} />
              </button>
            </div>
          ))}
        </div>
