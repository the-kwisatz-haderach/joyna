...
import { Cancel01Icon, UserAdd01Icon, Tick02Icon } from "@hugeicons/core-free-icons";
...
export function GuestRow({
  ...
  if (guest.isHost) {
    ...
    trailing = (
      ...
      >
        <HugeiconsIcon icon={Cancel01Icon} className="h-3 w-3 text-joyna-red" strokeWidth={2.5} />
      </button>
    );
    ...
    trailing = (
      <span className="flex shrink-0 items-center gap-1 rounded-full bg-joyna-mint/20 px-2.5 py-1 text-[10.5px] font-semibold text-joyna-mint-dark">
        <HugeiconsIcon icon={Tick02Icon} className="h-3 w-3" strokeWidth={2.5} /> Added
      </span>
    );
  } else if (isStranger) {
    trailing = (
      ...
      >
        <HugeiconsIcon icon={UserAdd01Icon} className="h-3 w-3" strokeWidth={2.2} /> Add
      </button>
    );
  } else if (guest.group) {
    trailing = <span className="text-[11px] text-joyna-ink-faint whitespace-nowrap">{guest.group}</span>;
  }

  return (
    <div className={cn("flex gap-2.5 py-2 text-sm", guest.reason && "items-start")}>
      <GuestAvatar name={guest.name} variant={avatarVariant} src={guest.avatarSrc} />
      {guest.reason ? (
        <div className="flex-1">
          ...
          <div className="mt-0.5 text-[11.5px] italic text-joyna-ink-faint">&ldquo;{guest.reason}&rdquo;</div>
        </div>
      ) : (
        <>
...
export function GuestGroupLabel({ children }: { children: React.ReactNode }) {
  return <div className="mt-3 mb-1 text-xs font-semibold text-joyna-ink-faint">{children}</div>;
}
